...
class VideosView(View):
    def get(self, request, id=None, *args, **kwargs):
        if id is None:
            videos = list(Videos.objects.values())
        else:
            videos = list(Videos.objects.filter(id=id).values())
    ...
    def post(self, request, *args, **kwargs):
        form_data = json.loads(request.body.decode())
        name, length = form_data['name'], form_data['length']
        p = Videos(name=name, length=length)
        p.save()
        return HttpResponse(status=200)
...
@method_decorator(csrf_exempt, name='dispatch')
class InvocationsView(View):
    def get(self, request, id=None, *args, **kwargs):
        if id is None:
            invocation = list(Invocations.objects.values())
        else:
            invocation = list(Invocations.objects.filter(id=id).values())
        return JsonResponse(invocation, safe=False)

    @retry_on_exception(3)
    @atomic
    def post(self, request, *args, **kwargs):
        form_data = json.loads(request.body.decode())
        c = Users.objects.get(id=form_data['user']['id'])
        o = Invocations(subtotal=form_data['subtotal'], customer=c)
        o.save()
        for p in form_data['videos']:
            p = Videos.objects.get(id=p['id'])
            o.videos.add(p)
        o.save()
        return HttpResponse(status=200)